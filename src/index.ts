...
const resolvers = {
    Query: {
      ...
      authors: () => authors,
      author: (_:any, {id}:{id:string}) => authors.find(author => author.id === id),
      authorBooks: (_:any, {id}:{id:string}) => books.filter(book => book.authorId === id)
    },

    Mutation: {
        addBook: (_:any, {book}:AddBookArgs) => {
            const newBook = {
